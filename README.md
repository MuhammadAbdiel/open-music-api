...
Dikerjakan oleh [Yoga Perdana Putra](https://github.com/MuhammadAbdiel)
...
Buat dulu database dengan nama `musicapi` pada PostgreSQL dan berikan hak akses ke user yang digunakan dengan kueri berikut melalui `psql`:

```sql
CREATE DATABASE musicapi;
GRANT ALL PRIVILEGES ON DATABASE musicapi TO <username>;
```
...
Untuk menjalankan aplikasi pada **_development_** _environment_, gunakan perintah berikut:

```sh
npm run start-dev
```

Untuk menjalankan aplikasi pada **_production_** _environment_, ubah _script_ pada [package.json](./package.json) dan variabel pada berkas [.env](./.env) dengan pengaturan _deployment_ sesuai server yang digunakan.

Gunakan perintah berikut pada `node` untuk membuat **_token_**:
